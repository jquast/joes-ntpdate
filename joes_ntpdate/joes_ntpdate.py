...
@retry()
def get_ntpserver_response(ntp_host):
    ntpc = ntplib.NTPClient()
    return ntpc.request(ntp_host)
...
def ntpdate(ntp_host,
    ...
    print('{0}: '.format(ntp_host), end='')
    sys.stdout.flush()

    resp = get_ntpserver_response(
        ntp_host, _retry_count=retries, _retry_delay=1, _retry_backoff=backoff_factor)
...
def parse_args(args):
    ...
    parser.add_argument('--backoff-factor', default=2.0,
...
def main():
    ...
    # re-map root logger for a basic console output, (for retryable.py
    # notifications)
    root_log = logging.getLogger(None)
    map(root_log.removeHandler, root_log.handlers)
